"""가짜 WCS 서버 검증용 가짜 로봇 (실제 로봇/SDK 없이 통신 시퀀스만 재현).
    ...
    python tools/fake_wcs/sim_robot.py                       # 서버 localhost:5224
    WCS_BASE_URL=http://localhost:5224 python tools/fake_wcs/sim_robot.py --work-sec 3 --cycles 2
    python tools/fake_wcs/sim_robot.py --error "테스트 오류"   # 첫 사이클을 ERROR로 끝내고 종료

데모(demo_full_sequence_loop.py)와 같은 통신 모듈(communication.wcs)을 그대로 쓴다:
IDLE(하트비트) → START 명령 폴링 → WORKING(work-sec) → DONE → IDLE … 반복.
"""
...
def main() -> int:
    ...
    parser.add_argument("--error", default=None, help="첫 사이클을 이 메시지의 ERROR로 끝내고 종료")
    ...
    try:
        ...
        while args.cycles == 0 or cycle <= args.cycles:
            publisher.set_work_state("IDLE")
            log.info("START 명령 대기 중...")
            command = publisher.wait_for_command("START")
            log.info("사이클 %d 시작 (commandId=%s)", cycle, command["commandId"])
    ...
    except Exception as error:  # noqa: BLE001 - 데모와 같은 방식으로 ERROR 보고
        publisher.set_work_state("ERROR", str(error))
        log.info("ERROR 보고: %s", error)
    finally:
        stop = True

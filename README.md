...
LOAD readiness RACK01_PORT01: READY (1회 조회, 0초) -> 로딩 시작
WCS transport-event OK: ARRIVED_AT_TO …                        To 도착 보고
UNLOAD readiness CV02_IN: READY (1회 조회, 0초) -> 언로딩 시작
사이클 1 완료
...
- **T-03 NOT_READY 후 READY**: PIO 카드에서 `RACK01_PORT01` LOAD를 NOT_READY로 두고 [오더 발행] → 로봇이 파지 전에 멈춰
  "NOT_READY — 2초마다 재확인"을 남기고 카드에 "로봇 대기 중 · N회"가 뜬다 → READY를 누르면 즉시 진행.
- **T-04 대기 초과**: `READINESS_MAX_WAIT_SEC=10`으로 로봇을 띄우고 UNLOAD를 NOT_READY로 두면 10초 후 로봇이
  `PIO_NOT_READY_TIMEOUT: UNLOAD CV02_IN 10s`를 message로 FAILED를 보고하고 서버는 HALTED가 된다.
...
curl 'localhost:5224/api/v1/rb/stations/RACK01_PORT01/readiness?operation=LOAD&wcsOrderId=X'      # 기본 READY
curl -X POST localhost:5224/api/test/readiness -H 'Content-Type: application/json' \
  -d '{"stationId":"CV02_IN","operation":"UNLOAD","status":"NOT_READY","reasonCode":"CONVEYOR_FULL"}'

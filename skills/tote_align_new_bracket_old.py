...
- RIGHT와 LEFT 추정이 일치하면 정밀한 RIGHT 모델을 우선 사용한다.
- 두 추정이 충돌하면 frame 수 또는 직전 이동의 예상 잔여오차로 오검출을 버린다.
...
WINDOW_NAME = "Tote Align - New Bracket Coupled Recovery v4"
...
def measure_feature(
    ...
    *,
    ...
    right_feature = (
        ...
        if len(right_rows) >= MIN_SIDE_FRAMES
        else None
    )
    left_feature = (
        np.median(np.asarray(left_rows, dtype=np.float64), axis=0)
        if len(left_rows) >= MIN_SIDE_FRAMES
        else None
    )
        ...
        if (
            ...
            if expected_error is not None:
                ...
                if (
                    ...
                    return None
            elif len(left_rows) - len(right_rows) >= SIDE_FRAME_ADVANTAGE:
                selected_side, selected_feature, selected_frames = (
                    "LEFT",
                    left_feature,
                    len(left_rows),
                )
            elif len(right_rows) - len(left_rows) >= SIDE_FRAME_ADVANTAGE:
                selected_side, selected_feature, selected_frames = (
                    "RIGHT",
                    right_feature,
                    len(right_rows),
...
def pose_error_to_command(error: PoseError) -> RelativeCommand:
    ...
    return RelativeCommand(
        ...
        y_m=+sin_theta * error.x_m - cos_theta * error.y_m,
        ...
        *,
        ...
        while correction_index <= MAX_CORRECTIONS:
            ...
            if measurement is None:
                ...
                print(
                    "Tote 정렬 실패: 정지 재측정 및 전방 recovery 후에도 "
                    "corner를 선택하지 못했습니다."
                )
                return True

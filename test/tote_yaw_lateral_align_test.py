#!/usr/bin/env python3
"""토트 TOP rim으로 yaw를 맞춘 뒤 TL/TR 중심으로 lateral을 맞춘다.

1. 같은 TOP detector로 angle 측정
2. yaw 한 번 이동
3. 같은 TOP + LEFT + RIGHT로 TL/TR 검출
4. center_x로 lateral 한 번 이동
5. 각각 이동 후 결과만 검증
...
def edge_support(edges: np.ndarray, start: np.ndarray, end: np.ndarray) -> float:
    """예상 직선 주변에 실제 edge가 얼마나 있는지 확인한다."""
    height, width = edges.shape
...
def detect_fine(image: np.ndarray) -> FineFeature | None:
    ...
    # 같은 토트에서 안쪽 edge보다 실제 외곽 side를 고르기 위해 가장 넓은 TOP 조합을 사용한다.
...
def measure_fine(pipeline: rs.pipeline, title: str) -> FineMeasurement | None:
    """FINE feature를 여러 frame 측정하고 median center_x를 사용한다."""
    features = []
    start_time = time.monotonic()
    ...
    filtered = [
        ...
        if abs(feature.center_y_px - median_y) <= FINE_Y_FILTER_PX
...
def move_yaw(robot, monitor: OdometryMonitor, angle_deg: float) -> bool:
...
def move_lateral(robot, monitor: OdometryMonitor, distance_m: float) -> bool:
    leg = build_leg(
        start=odom_pose(monitor.odom),
        target=(0.0, distance_m, 0.0),
        absolute=False,
...
def align_yaw(robot, monitor: OdometryMonitor, pipeline: rs.pipeline) -> bool:
    ...
    if not move_yaw(robot, monitor, command_deg):
        return False
...
def align_lateral(robot, monitor: OdometryMonitor, pipeline: rs.pipeline) -> bool:
    ...
    if not move_lateral(robot, monitor, command_m):
        return False
